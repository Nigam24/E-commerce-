...
function Cart() {
  ...
  const { data, error } = useFetch(isLoggedIn ? `/add/cart` : null); // Fetch only if logged in
    ...
    <>
      ...
      {showOrder ? (
        ...
        <div className="shopping-cart">
          {!isLoggedIn ? ( // If not logged in, show empty cart
            <div className="empty-cart">
              <h2>Your Cart is Empty</h2>
              <p>Please log in to view your cart items.</p>
            </div>
          ) : cartItems && cartItems.length > 0 ? ( // Show cart items only if available
            <>
